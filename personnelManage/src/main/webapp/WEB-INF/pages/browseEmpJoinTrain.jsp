    ...
    <script type="text/javascript">
        function departSelect(data) {
            ...
            if(data.value==${department.depart_no}){
                ...
                <c:forEach items="${sessionScope.joinEmployees}" var="emp">
                ...
                if (data.options[index].innerHTML=="${emp.depart_name}"){
                    ...
                    cell6.innerHTML="<input name='ridEmp' type='checkbox' value='${emp.e_id}'>选择";
                    cell7.innerHTML="<a href='trainRecord?train_no=${train.train_no}'>培训记录</a> ";
                }
                </c:forEach>
            }
            </c:forEach>
...
<div>
    <form method="post" action="ridEmpFromTrain">
    ...
    <table id="empTable">
        ...
        <c:forEach items="${sessionScope.joinEmployees}" var="employee">
            <tr>
                <td>${employee.e_id}</td><td>${employee.e_name}</td>
                <td>${employee.gender}</td>
                <td>${employee.depart_name}</td><td>${employee.pos_name}</td>
                <td><input name="ridEmp" type="checkbox" value="${employee.e_id}">剔除</td>
                <td><a href="trainRecord?train_no=${train.train_no}&e_id=${employee.e_id}">培训记录</a> </td>
                </tr>
        </c:forEach>
    </table>
